...
static Character typeface[] =
{
  'a', {0x7c,0x44,0x44,0x7c,0x44},
  // we put a dummy at the top of the array.
  // but why oh why is this required???
  // it seems that on every call to getCharacter(), the first item is set
  // to the previous call result

  // typeface posted by Andreas (b00lean) on arduino forum
  // http://www.arduino.cc/cgi-bin/yabb2/YaBB.pl?num=1234005825/0
  // 5x5 characters padded with one bit on the left and two on the right
'a', {0x7c,0x44,0x44,0x7c,0x44},
...
'0', {0x7c,0x4c,0x54,0x64,0x7c},
'1', {0x10,0x30,0x10,0x10,0x38},
'2', {0x78,0x04,0x38,0x40,0x7c},
'3', {0x7c,0x04,0x38,0x04,0x7c},
'4', {0x40,0x40,0x50,0x7c,0x10},
'5', {0x7c,0x40,0x78,0x04,0x78},
'6', {0x7c,0x40,0x7c,0x44,0x7c},
'7', {0x7c,0x04,0x08,0x10,0x10},
'8', {0x7c,0x44,0x7c,0x44,0x7c},
'9', {0x7c,0x44,0x7c,0x04,0x7c},
...
Character& getCharacter(char c){
  int pos = 0;
  Character& match = typeface[pos];
  match = typeface[++pos];
  // otherwise matc === previous match... really strange behaviour!
  while(match.c != c && match.c != '\0')
    match = typeface[++pos];
  return match;
}
